    ...
    I: IntoIterator<Item = &'a PublicKey>,
{
    ...
    // FORK PATCH 6 (see PATCHES.md): the parsed `signed` value is kept as well,
    // for the deserialization at the end of this function.
    let (signatures, signed_value, canonical_bytes) = {
        #[derive(Deserialize)]
        ...
        (unverified.signatures, unverified.signed, canonical_bytes)
    };
    ...
    // value the canonical bytes were produced from, rather than from `raw_meta.as_bytes()`.
    //
    // FORK PATCH 6 (see PATCHES.md): upstream re-parsed `canonical_bytes` here.
    // Canonical JSON emits control characters literally, and a strict JSON
    // parser rejects those, so metadata containing a newline inside a string
    // -- a PEM-encoded key, say -- could not be re-parsed at all.
    //
    // The security property is unchanged. Canonicalization is a pure function
    // of this value, so this is exactly the data whose canonical form was just
    // verified, and no unsigned bytes from the raw document can leak in.
