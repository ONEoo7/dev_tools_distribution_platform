...
/// FORK PATCH 1 (see PATCHES.md): compare the MAJOR spec version only.
///
/// The TUF specification requires clients to check that the MAJOR version is
/// compatible. Upstream compared the whole string against "1.0", which rejects
/// metadata from python-tuf, whose `spec_version` carries the precise
/// specification version such as "1.0.31".
fn check_spec_version(found: &str) -> Result<()> {
    fn major(version: &str) -> &str {
        version.split('.').next().unwrap_or("")
    }

    if major(found).is_empty() || major(found) != major(SPEC_VERSION) {
        return Err(Error::Encoding(format!("Unknown spec version {}", found)));
    }
...
pub struct Delegation {
    // FORK PATCH 3 (see PATCHES.md): the specification names this field
    // `name`; upstream serialised it as `role`, so it could not read metadata
    // from any conforming implementation.
    #[serde(rename = "name")]
    ...
    #[serde(rename = "keyids")]
    key_ids: Vec<crypto::KeyId>,
    // FORK PATCH 2d (see PATCHES.md): raw strings, so a delegation path may
    // carry glob metacharacters that TargetPath::new rejects for real targets.
    paths: Vec<String>,
}
...
impl Delegation {
    ...
    pub fn try_into(self) -> Result<metadata::Delegation> {
        let mut parsed = Vec::with_capacity(self.paths.len());
        for path in &self.paths {
            parsed.push(metadata::TargetPath::new_pattern(path.clone())?);
        }
        let paths = parsed
            .into_iter()
