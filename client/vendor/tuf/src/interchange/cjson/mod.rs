    ...
    fn write(&self, buf: &mut Vec<u8>) -> std::result::Result<(), String> {
        match *self {
            ...
            Value::Number(Number::U64(n)) => itoa::write(buf, n)
                ...
                .map_err(|err| format!("Write error: {}", err)),
            Value::String(ref s) => {
                // FORK PATCH 6 (see PATCHES.md): Canonical JSON escapes only
                // the double quote and the backslash. Every other character,
                // control characters such as newline included, is emitted
                // literally.
                //
                // Upstream delegated to serde_json, which escapes far more.
                // That changes the bytes that get hashed and signed, so any
                // metadata carrying a string with a newline -- a PEM-encoded
                // key, for instance -- produced the wrong key ID and failed
                ...
                for c in s.chars() {
                    ...
                    buf.extend(c.encode_utf8(&mut encoded).as_bytes());
                }
                buf.push(b'"');
                Ok(())
            }
...
mod test {
    ...
    fn write_obj() {
        ...
        let mut out = Vec::new();
        jsn.write(&mut out).unwrap();
        // FORK PATCH 6 (see PATCHES.md): Canonical JSON emits the newline
        // literally. Upstream asserted the JSON-escaped "\\n" here, which is
        // what made key IDs and signatures disagree with every other
        // implementation for any string containing a control character.
