...
use {
    ...
    ring::{
        ...
        rand::SystemRandom,
        // FORK PATCH 5 (see PATCHES.md): ECDSA P-256 verification.
        signature::{Ed25519KeyPair, KeyPair, ECDSA_P256_SHA256_ASN1, ED25519},
    },
    serde::{
...
/// FORK PATCH 5 (see PATCHES.md): OIDs for ECDSA on the NIST P-256 curve.
/// id-ecPublicKey, 1.2.840.10045.2.1
const EC_PUBLIC_KEY_SPKI_OID: &[u8] = &[0x2a, 0x86, 0x48, 0xce, 0x3d, 0x02, 0x01];
/// prime256v1 (secp256r1 / NIST P-256), 1.2.840.10045.3.1.7
const P256_CURVE_OID: &[u8] = &[0x2a, 0x86, 0x48, 0xce, 0x3d, 0x03, 0x01, 0x07];
/// Uncompressed EC point: 0x04 followed by two 32-byte coordinates.
const P256_UNCOMPRESSED_POINT_LENGTH: usize = 65;
...
/// FORK PATCH 5 (see PATCHES.md): extract the uncompressed EC point from a
/// PEM-encoded P-256 SubjectPublicKeyInfo.
///
/// `ring` wants the raw point, but TUF metadata carries the PEM. The PEM is
/// what gets stored, because the key ID is the hash of the canonical JSON of
/// the key object and must round-trip byte for byte.
///
/// Both OIDs are checked. Accepting the point without confirming the curve
/// would let a key on some other curve be verified as though it were P-256.
fn ecdsa_p256_point_from_pem(pem: &[u8]) -> Result<Vec<u8>> {
    let pem = std::str::from_utf8(pem)
        .map_err(|_| Error::Encoding("ECDSA public key is not valid UTF-8".into()))?;
    ...
    let der = BASE64
        ...
        .map_err(|e| Error::Encoding(format!("ECDSA public key is not valid base64: {:?}", e)))?;
    ...
    let point = input
        .read_all(derp::Error::Read, |input| {
            derp::nested(input, Tag::Sequence, |input| {
                derp::nested(input, Tag::Sequence, |input| {
                    let algorithm = derp::expect_tag_and_get_value(input, Tag::Oid)?;
                    if algorithm.as_slice_less_safe() != EC_PUBLIC_KEY_SPKI_OID {
                        return Err(derp::Error::WrongValue);
                    }
                    let curve = derp::expect_tag_and_get_value(input, Tag::Oid)?;
                    if curve.as_slice_less_safe() != P256_CURVE_OID {
                        return Err(derp::Error::WrongValue);
                    }
                    Ok(())
                })?;
                let point = derp::bit_string_with_no_unused_bits(input)?;
                Ok(point.as_slice_less_safe().to_vec())
            })
        })
        .map_err(derp_error_to_error)?;

    if point.len() != P256_UNCOMPRESSED_POINT_LENGTH || point.first() != Some(&0x04) {
        return Err(Error::Encoding(
            "ECDSA public key is not an uncompressed P-256 point".into(),
        ));
    }
...
pub enum SignatureScheme {
    ...
    /// FORK PATCH 5 (see PATCHES.md): ECDSA over NIST P-256 with SHA-256 and
    /// ASN.1 signatures. Verification only; this crate cannot sign with it.
    EcdsaSha2Nistp256,
...
pub enum KeyType {
    ...
    /// FORK PATCH 5 (see PATCHES.md): ECDSA. Verification only.
    Ecdsa,
...
impl KeyType {
    ...
    fn as_oid(&self) -> Result<&'static [u8]> {
        match *self {
            ...
            // FORK PATCH 5 (see PATCHES.md): ECDSA keys are held in their
            // original PEM form, so this crate never re-encodes them as SPKI.
            KeyType::Ecdsa => Err(Error::IllegalArgument(
                "cannot write an ECDSA key as SPKI".into(),
            )),
            #[cfg(feature = "unstable_rsa")]
...
impl PublicKey {
    ...
    pub fn verify(&self, role: &MetadataPath, msg: &[u8], sig: &Signature) -> Result<()> {
        // FORK PATCH 5 (see PATCHES.md): ECDSA keys are stored as PEM rather
        // than as raw key bytes, so the point is extracted here instead of
        // being handed straight to `ring` like the other schemes.
        if self.scheme == SignatureScheme::EcdsaSha2Nistp256 {
            let point = ecdsa_p256_point_from_pem(&self.value.0)?;
            let key =
                ring::signature::UnparsedPublicKey::new(&ECDSA_P256_SHA256_ASN1, point.as_slice());
            return key
        ...
        let alg: &dyn ring::signature::VerificationAlgorithm = match self.scheme {
            ...
            SignatureScheme::EcdsaSha2Nistp256 => unreachable!("handled above"),
...
impl<'de> Deserialize<'de> for PublicKey {
    fn deserialize<D: Deserializer<'de>>(de: D) -> ::std::result::Result<Self, D::Error> {
        ...
        let key = match intermediate.keytype() {
            ...
            // FORK PATCH 5 (see PATCHES.md): keep the PEM exactly as it
            // arrived. The key ID is the hash of the canonical JSON of the key
            // object, so re-encoding the key here would change the key ID and
            // every signature reference would stop matching.
            KeyType::Ecdsa => {
                if intermediate.scheme() != &SignatureScheme::EcdsaSha2Nistp256 {
                    return Err(DeserializeError::custom(format!(
                        "ecdsa key type must be used with the ecdsa-sha2-nistp256 \
                         signature scheme, not {:?}",
                        intermediate.scheme()
                    )));
                }
                ...
                .map_err(|e| {
                    DeserializeError::custom(format!("Couldn't parse key as ECDSA: {:?}", e))
                })?;

                // Reject a malformed key at parse time rather than at the first
                // signature check, so a bad key cannot sit unnoticed in root.
                ecdsa_p256_point_from_pem(intermediate.public_key().as_bytes())
                    .map_err(|e| DeserializeError::custom(format!("{:?}", e)))?;
