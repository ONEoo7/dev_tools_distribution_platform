    ...
    pub fn target_description_with_start_time(
        ...
        fn lookup<D: DataInterchange>(
            ...
            for delegation in delegations.roles() {
                ...
                // FORK PATCH 4 (see PATCHES.md): SECURITY. Upstream skipped this
                // check entirely at depth 0, and at greater depths checked
                // `parents`, which excludes the current delegation's own paths.
                // A delegated role's `paths` were therefore never enforced
                // against its own targets, so any delegated role could sign any
                // target path. Check `new_parents`, which includes them, at
                // every depth.
