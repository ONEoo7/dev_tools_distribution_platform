...
fn safe_path(path: &str) -> Result<()> {
    safe_path_inner(path, false)
}

/// FORK PATCH 2b (see PATCHES.md): glob match within a single path segment.
///
/// Supports `*` (any run of characters, never crossing a segment because the
/// caller has already split on `/`) and `?` (exactly one character). Iterative
/// with backtracking, so a pattern full of wildcards cannot blow the stack.
...
/// FORK PATCH 2a (see PATCHES.md): validation for delegation PATHPATTERNs.
///
/// A delegation path is a pattern, not a filename, so the glob metacharacters
/// `*` and `?` are legal in it even though they are rejected in a real target
/// path for filesystem compatibility.
fn safe_pattern(path: &str) -> Result<()> {
    safe_path_inner(path, true)
}

fn safe_path_inner(path: &str, allow_glob: bool) -> Result<()> {
    if path.is_empty() {
...
impl TargetPath {
    ...
    /// FORK PATCH 2a (see PATCHES.md): build a delegation PATHPATTERN.
    ///
    /// Same validation as [`TargetPath::new`] except that the glob
    /// metacharacters `*` and `?` are permitted, because a delegation path is
    /// a pattern rather than a filename. Match with
    /// [`TargetPath::matches_pattern`].
    pub fn new_pattern<P: Into<String>>(path: P) -> Result<Self> {
        let path = path.into();
        safe_pattern(&path)?;
        Ok(TargetPath(path))
    }

    /// FORK PATCH 2b (see PATCHES.md): specification-conformant PATHPATTERN match.
    ///
    /// The TUF specification matches a delegation path pattern against a
    /// target path segment by segment, where `*` matches any run of characters
    /// within one segment and `?` matches exactly one. Upstream implemented
    /// only directory-prefix matching via [`TargetPath::is_child`], which
    /// cannot express a pattern such as `app/*/*/*/*` and rejects it at parse
    /// time.
    ///
    /// Upstream's prefix form is still accepted, so metadata that relies on it
    /// keeps working.
    pub fn matches_pattern(&self, pattern: &Self) -> bool {
        if self == pattern || self.is_child(pattern) {
            return true;
        }

        let target: Vec<&str> = self.0.split('/').collect();
        let pattern: Vec<&str> = pattern.0.split('/').collect();

        // A pattern only ever matches a path with the same number of segments;
        // there is no recursive wildcard in the specification.
    ...
    pub fn matches_chain(&self, parents: &[HashSet<TargetPath>]) -> bool {
        ...
        if parents.len() == 1 {
            return parents[0].iter().any(|p| self.matches_pattern(p));
        }
            ...
            .map(|group| {
                group
                    ...
                    .filter(|parent| {
                        parents[0]
                            .iter()
                            .any(|p| parent.matches_pattern(p))
                    })
                    .cloned()
                    .collect::<HashSet<_>>()
            })
    ...
    fn serde_targets_with_delegations_metadata() {
        ...
        let jsn = json!({
            ...
            "delegations": {
                ...
                "roles": [
                    {
                        // FORK PATCH 3 (see PATCHES.md): the specification
                        // calls this field `name`, not `role`.
                        "name": "foo/bar",
                        "terminating": false,
                        "threshold": 1,
                        "keyids": ["a9f3ebc9b138762563a9c27b6edd439959e559709babd123e8d449ba2c18c61a"],
